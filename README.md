...
| `backfill_minutes` | 断线重连时回补最近 N 分钟错过的推文（0-5），默认关闭 |
| `credits_balance` | 控制台当前显示的 credits 余额（X 无余额查询 API，手动填入作为估算基准，余额变动后更新即重置基准），0 表示不启用 |
| `credit_cost_per_post` | 控制台标注的每条 Post credits 单价，用于剩余 credits 估算 |
...
| `/xfwd status` | 所有人 | 查看流连接状态、剩余额度、累计转发数和所有会话的订阅情况 |
| `/xfwd usage` | 所有人 | 查看当日 / 本周 / 本月的计费条数与剩余额度 |
...
| `/xfwd help` | 所有人 | 显示指令帮助；`/xfwd` 后跟未知子指令或无子指令时也会自动回复帮助 |

新增/删除流规则等管理操作均在 WebUI 插件页面完成（需登录管理面板）。普通用户需要订阅规则未覆盖的新用户时，请联系管理员添加规则。
...
- **消费统计**：最近 30 天每日计费条数柱状图 + 按规则 ID 的累计计费条数表（含已删除规则）。pay-as-you-go 下每条投递的推文计费一次，插件按命中的规则 ID 本地计数并持久化保存 90 天（`usage.json`）；
- 状态卡片显示两个余额指标，`/xfwd status` / `/xfwd usage` 同样显示：
  - **剩余额度 (Post)**：`GET /2/usage/tweets` 的月度上限 − 已用（10 分钟缓存）；
  - **剩余 Credits（估算）**：X 未提供 credits 余额查询端点，需在配置中填入控制台余额（`credits_balance`）与单价（`credit_cost_per_post`），插件按 基准余额 − 此后接收条数 × 单价 实时估算；只统计本插件流式接收的消耗，其他 API 调用不计入。
